...
def lambda_handler(event, context):
    ...
    if not trigger:
        ...
        to_email, from_email = getEmails(accountName)
        account_id = context.invoked_function_arn.split(":")[4]
        sesArn = 'arn:aws:ses:us-east-1:'+account_id+':identity/' + from_email
        subject = get_subject1(account_id)
        email, logmessage = build_email1(account_id)
...
def get_subject(trigger, account_id):
    subject = 'AWS '+trigger+'-alert for account: ' +str(getAccountName())+' ('+str(account_id)+') '
    return subject
def build_email(events, trigger, account_id):
    email = buildEmailHeader(trigger, account_id)
    logmessage = 'The '+trigger+'-Alarm  on account '+str(getAccountName())+' ('+str(account_id)+') has been triggered because of following actions: \n \n'
    for event in events:
...
def buildEmailHeader(trigger, account_id):
    ...
    header += '<p>this is an automated '+trigger+'-alert for the account '+str(getAccountName())+' ('+str(account_id)+') which was triggered because of the following actions:.<br> </p>'
...
def getAccountName():
    ...
    for response in paginator.paginate():
        accountName=accountName+response['AccountAliases'][0]
        print(response['AccountAliases'])
    return accountName
def getEmails(accountName):
    account=accountName.split('-')[0]
    if '-mt' not in accountName:
            to_email='cloudops@allianz.de'
            from_email='cloudops@allianz.de'
        else:
            account=account.split('-')[0]
            to_email=account+'.cloud@allianz.de'
            from_email=account+'.cloud@allianz.de'
